...
if (IgnorePrevious) {
    ...
    # Analogy2.wide <- Analogy2.wide %>% # data already in wide format
...
if (showAttentionCheck) {
    AttentionCheck.df <- Analogy2.df %>%
      group_by(subjID) %>%
      slice_head(n=1) %>%
      dplyr::select(subjID, attn_check_missed, attn_check)  
    Problems.df <- subset(AttentionCheck.df, attn_check_missed > 0)
    if (nrow(Problems.df) > 0)
        # print(as.data.frame(subset(AttentionCheck.df, attn_check_missed > 0, attn_check)))
      print(as.data.frame(Problems.df[, c("subjID", "attn_check", "attn_check_missed")]))
    else {
...
SummarizedInfo.df <- SummarizeParticipants(Analogy2.df)
