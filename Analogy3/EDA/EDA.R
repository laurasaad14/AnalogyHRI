...
# qualitative data
emo_exp <- Analogy3.df %>%
  select(subjID, condition, CRDB_emoexp, DRCB_emoexp, conPos1EMO)
...
write.csv(emo_exp, file = "analogy3_emotion_ranking_explanation.csv")


emo_sarah <- Analogy3.df %>%
  select(subjID, howemot)
